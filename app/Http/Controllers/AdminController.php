...
// if captcha is OK
    ...
    public function paymentGateways(){
        ...
        $data=DB::table('paymentMethods')->get();
        return view::make('admin.payments.paymentList',['data'=>$data]);
    }
    public function paymentGatewaysUpdate($id){

        $updateAraay=['Name'=>$_POST['name'],'content'=>\json_encode($_POST),'status'=>(isset($_POST['status'])&&$_POST['status']=='on')?"1":"0"];
        DB::table('paymentMethods')->where('id',$id)->update($updateAraay);
        return Redirect::back();
    }
    ...
    public function Headersetup()
    {
        if (Request::isMethod('post')) {
            ...
            try {
                ...
                $page->slider_text = serialize($data['header']);
                $page->save();
            } catch (Exception $e) {
                Session::put('fail', '1');
                //return $e;
...
//dd(unserialize($data->slider_text));
        $item = [];
        if (!empty($data->slider_text))
        ...
        if (Request::isMethod('post')) {
            ...
            try {
                ...
                switch ($data['footer']['type']) {
                    case "title":
                        $arr['title'] = $data['footer'];
                        break;
					case "description":
                        $arr['description'] = $data['footer'];
                        break;
					case "copyright":
                        $arr['copyright'] = $data['footer'];
                        break;
					case "about":
                        $arr['about'] = $data['footer'];
                        break;
					case "newsletter":
                        $arr['newsletter'] = $data['footer'];
                        break;
					case "address":
                        $arr['address'] = $data['footer'];
                        break;
					case "contact_no":
                        $arr['contact_no'] = $data['footer'];
                        break;
					case "fax":
                        $arr['fax'] = $data['footer'];
                        break;
					case "email_1":
                        $arr['email_1'] = $data['footer'];
                        break;
					case "email_2":
                        $arr['email_2'] = $data['footer'];
                        break;
                    case "image":
                        if (!empty($data['fg'])) {
                            $file = Input::file('fg');
...
//dd($fname);
                    ...
                    case "icon":
                        // Edit or Add
                        if (isset($data['footer']['edit-icon']) && !isset($data['footer']['delete-icon'])){

                            $arr['icon'][$data['footer']['edit-icon']] = $data['footer'];
                        }

                        else if(isset($data['footer']['delete-icon']) && !isset($data['footer']['edit-icon'])){
                            unset($arr['icon'][$data['footer']['delete-icon']]);
                        }

                        else if(isset($data['footer']['all_delete-icon']) && !isset($data['footer']['edit-icon'])){
                            $selectet_id = explode(',', $data['footer']['all_delete-icon']);
                            foreach ($selectet_id as $key => $value) {
                                unset($arr['icon'][$value]);
                            }
                        }
