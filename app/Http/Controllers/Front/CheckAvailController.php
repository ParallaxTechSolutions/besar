...
class CheckAvailController extends Controller {
    ...
    function index() {
        $data = Input::all();
        $products = array();
        if (isset($data['property'])  && isset($data['arrival']) )
        $products = \App\Models\Product::where('property_id',$data['property'])->where('drop_list_id',$data['arrival'])->get();
...
//dd($data);
    ...
    function check_avail() {
        $data = Input::all();
        // if($data['product_id'] == '') {
        //     return json_encode(['status' => true, 'avail' => [], 'product_details' => [], 'package' => []]);
        // }
        $rules = [
        ...
        if (!empty($data['product_id'])) {
            ...
            $packages = DB::table('product_to_quantity_discount')->where('product_id',$data['product_id'])->get();
            if(count($packages) > 0){
                $package = $packages;
            } else {
                $package = [];
      ...
      $checkInDay = strtotime($data['checkin_date']);
      $checkInDay = date('D', $checkInDay);

      $checkOutDay = strtotime($data['checkout_date']);
      $checkOutDay = date('D', $checkOutDay);

      echo json_encode(['status' => true, 'avail' => $res, 'product_details' => $product_details, 'package' => $package, 'checkInDay' => $checkInDay, 'checkOutDay' => $checkOutDay]);
