           ...
           $data = Input::all();
            ...
            if ($this->ContactusModel->createContact($contact)) {
               ...
               $messageData = [
					...
					$notificationsArr = $notification->where('actions', 'like', '%feedback_received%')->where('status',1)->get();
					foreach($notificationsArr as $notifications){
						$sentEmailArr = array();
						$emailsArr = explode(',',$notifications->emails);
						foreach($emailsArr as $email){
							if(!in_array($email, $sentEmailArr)){
								Mail::send('emails.contactus', ['contact' => $contact], function ($message) use ($messageData, $email, $notifications) {
