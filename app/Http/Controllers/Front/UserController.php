...
class UserController extends Controller {
    ...
    ////Get PArtner order detail
    public function partnerOrderdetails($id){
        if(Session::get('userId')!='' and Session::get('userEmail')!=''){
            $this->data['page_title'] = 'Order Detail';
    ...
    {
        $customer=Customer::where('email',Session::get('userEmail'))->first();
        $order=Order::where('id',$request->booking_id)->first();
        $partner=Partners::where('id',$order->partner_id)->first();
//        dd($request->all());
        $feedback = DB::table('feedbacks')->insert([
            'name'=>$request->customer_name,
            'company_name'=>$partner->first_name . $partner->last_name,
            'company_address'=>$partner->address,
            'country'=>'Malaysia',
            'email'=>$customer->email,
            'contact_number'=>$customer->telephone,
            'post_code'=>$customer->code,
            'questions_comments'=>$request->feedbacktext,
        ]);
