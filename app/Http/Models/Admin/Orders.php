...
//        $orders->leftjoin('partners', 'id', '=', 'partner_id');
...
//dd($orderToProduct->get());
    ...
    public function getOrderToProductForPartner($order_id) {
        $orderToProduct = DB::table('order_to_product as otp');
        $orderToProduct->leftjoin('colors as c', 'c.id', '=', 'otp.color_id');
        $orderToProduct->leftjoin('products as p', 'p.id', '=', 'otp.product_id');
        $orderToProduct->leftjoin('special_events as se', 'se.id', '=', 'otp.special_event_id');
...
//dd($orderToProduct->get());
    ...
    public function updatePartnerOrderStatus($request) {
//        dd($request);
        $update = [
        ...
        $updatedata=Order::where('order_id', $request['order_id'])->update($update);

        $updateOrderId=Order::where('order_id', $request['order_id'])->first();
        $orderId=$updateOrderId->id;
        if ($request['status'] == 'Cancelled' || $request['status'] == 'Declined') {
            $orders = DB::table('order_to_product as otp');
            $orders->select('otp.*', 'p.quantity_in_stock');
            $orders->leftjoin('products as p', 'p.id', '=', 'otp.product_id');
            $orders->where('otp.order_id', '=', $orderId);
            ...
            foreach ($products as $product) {
                ...
                $updatedata = Product::where('id', $product->product_id)->update($update);
            }
        }
            $driverDetails=DB::table('driver_details')->where('order_id',$orderId)->first();
//            dd($driverDetails->order_id);
            if ($driverDetails)
            {
                $driverDetailinsert=DB::table('driver_details')->insert([
                    'partner_id'=> Session::get('userId'),
                ...
                $driverDetailupdate=DB::table('driver_details')->where('order_id',$orderId)->update([
                    'partner_id'=> Session::get('userId'),
                    'driver_name'     => $request['driver_name'],
                    'car_plate'       => $request['car_plate'],
                    'telephone_number' => $request['telephoneNumber'],
    ...
    public function updateOrderPaymentStatus($id, $status) {
        ...
        DB::table('orders')->where('order_id', $id)->update($update);

    }

    public function updatePartnerStatus($pid,$oid) {
        $update = [
            'partner_id' => $pid,
            'modifydate' => date('Y-m-d H:i:s')
        ];
        DB::table('orders')->where('id', $oid)->update($update);
    }
    ...
    public function getOrderData()
    {
        ...
        if(isset($data['start']) && isset($data['end'])) {
            $orders->where('date_checkin', '>=', date('Y-m-d', $data['start']))
                ->where('date_checkin', '<', date('Y-m-d', $data['end']));
        }
        $orders = $orders->whereIn('payment_status', ['Paid','Processing'])
