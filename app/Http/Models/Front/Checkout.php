...
class Checkout extends Model {
    ...
    public function addOrder($data) {
        ...
        $insert = [
            ...
            'tax_name' => $data['tax_name'].','.$data['tax_rate'],
            'ota_checklist_id' => isset($data['ota_checklist_id']) ? $data['ota_checklist_id']: 0,
        ];
        ...
        if($data['payment_method'] == "IPAY88"){
            $order_id = 'THRA' .$orderId; //sprintf('%05d', $orderId);
        }else{
        ...
        foreach ($data['products'] as $products) {
            ...
            $insert = [
                ...
                'product_bed' => $products['bed'],
                'product_guest' => $products['guest'],
                'product_meal' => $products['meal'],
                'product_promo_behaviour' => $products['promo_behaviour'],
                'product_thumb' => $products['product_thumb'],
                'order_id' => $orderId,
                'gst_rate' => $data['tax_rate'],
                'gst' => isset($data['tax_amount']) ? $data['tax_amount'] : 0,
                'product_id' => $products['product_id'],
                'quantity' => $data['room'],
                'color_id' => ($products['color_id'] ? $products['color_id'] : ''),
                'special_event_id' => $products['special_event_id'],
                'amount' => $products['amount'],
                'amount_sold_at' => $products['amount']/$products['quantity'],
                //'amount' => ($products['amount']*$data['room']),
            ...
            $insertRooms = [
                'date_checkin' => $products['date_checkin'],
                'date_checkout' => $products['date_checkout'] ,
                'order_id' => $orderId,
                'product_id' => $products['product_id'],
                'room_orderd'=>$data['room']
                ];

            DB::table('order_to_product')->insert($insert);
            DB::table('room_booked_date')->insert($insertRooms);

            //update product quantity
            $affected = DB::update("update products set quantity_in_stock = (quantity_in_stock - " . $products['quantity'] . "), last_modified = '" . date('Y-m-d H:i:s') . "' where id = ?", [$products['product_id']]);
            // echo "<pre>";
            // echo $affected;
            // echo "string";
            // udpate qty for product_room_prices
    ...
    public function updateOrderByRefNo($refno, $data) {
        ...
        /* get booking details to update room booking qty */
        $order_id = DB::table('orders')->where('order_id', $refno)->first()->id;
        $orders = DB::table('room_booked_date');
        $orders->select('*');
        // $orders->where('order_id', '=', $refno);
        $orders->where('order_id', '=', $order_id);
        $products = $orders->first();
        // dd($products);
        // udpate qty for product_room_prices
        $affected = DB::update("update product_room_prices set qty_stock = (qty_stock - 1), updated_at = '" . date('Y-m-d H:i:s') . "' where product_id = ? and date BETWEEN ? and ?", [$products->product_id, $products->date_checkin, date('Y-m-d', strtotime($products->date_checkout. ' - 1 days'))]);
        /**/

    }
    public function updateOrderByRefNoCC($refno, $data) {
        /* echo $refno;
        exit; */
        $data['modifydate'] = date('Y-m-d H:i:s');
        DB::table('orders')->where('id', $refno)->update($data);
        /* get booking details to update room booking qty */
        $order_id = DB::table('orders')->where('id', $refno)->first()->id;
        $orders = DB::table('room_booked_date');
        $orders->select('*');
        // $orders->where('order_id', '=', $refno);
        $orders->where('order_id', '=', $order_id);
    ...
    *
    */
    public function getOrderBookingDateByOrderId($order_id)
    {
        $orders = DB::table('room_booked_date as o');
        $orders->select('o.*');
        $orders->where('order_id', '=', $order_id);
    ...
    */
    ...
    public function getOrderByRefNoCC($refno) {
        $orders = DB::table('orders as o');
        $orders->select('o.*', 'sb.name as billing_state_name', 'cb.name as billing_country_name', 'ss.name as shipping_state_name', 'cs.name as shipping_country_name');
    ...
    public function getOrderToProduct($order_id) {
        ...
        $orderToProduct->select('otp.*', 'p.type', 'p.room_code', 'p.thumbnail_image_1', 'p.thumbnail_image_2', 'c.title as color_name', 'se.event_type', 'se.token', 'p.bed', 'p.guest', 'p.meal', 'pr.name');
    ...
    public function orderPlacement($payment, $payment_id) {
        ...
        //$priceByDates = $checkAvailModel->getPriceByDates($value->id, $data['checkin_date'], $data['checkout_date']);
        //New Code (RJ) END
        ...
        $orderObj->special_requests = $cart['special_requests'];
        $orderObj->tax_name = Session::get('tax_name').','.Session::get('tax_rate');
        $orderObj->save();

        $OrderToProductArr = [];
        $OrderProductQuantityFinish = [];
        $OrderProductQuantityLowArr = [];
        foreach ($cart['product'] as $room) {
            ...
            $OrderToProduct->quantity = $cart['rooms'];
            $OrderToProduct->amount = $room['sale_price'];
            $OrderToProduct->amount_sold_at = $room['sale_price']/$room['quantity'];
            ...
            $OrderToProduct->discount = $orderObj->discount;
            $OrderToProduct->promo_code_discount = $this->calculateDiscount([$room], $promo);
            // later added by sagar
            $OrderToProduct->product_type  = $room['type'];
            $OrderToProduct->product_code = $room['room_code'];
            $OrderToProduct->product_bed = $room['bed'];
            $OrderToProduct->product_guest = $room['guest'];
            $OrderToProduct->product_meal = $room['meal'];
            $OrderToProduct->product_promo_behaviour = $room['promo_behaviour'];
            $OrderToProduct->product_thumb = $room['thumbnail_image_1'];
            // dd($OrderToProduct);
            $OrderToProduct->save();
            ...
            //Amrir
            $RoomBookedDate->room_orderd = $cart['rooms'];
            //Amir code end
            //New Code (RJ) Start
            $priceByDates = $checkAvailModel->getPriceByDates($room['product_id'], $cart['arrival'], $cart['departure']);
            //New Code (RJ) END
            $RoomBookedDate->price_details = json_encode($priceByDates);
            ...
            ///////////
            $productArr = myProduct::where('id', $room['product_id'])->first()->toArray();
            $messageData = [
                'fromEmail' => 'registration@towerregency.com.my',
                'fromName' => 'Tower Regency Hotels & Apartments Online Booking',
                'subject' => 'No Room Active::Order #' . $orderObj->order_id
            ];

            $productRoomPriceStockQty = DB::table('product_room_prices')->whereBetween('date', [$cart['arrival'], $cart['departure']])->where('status',1)->where('product_id',$room['product_id'])->lists('qty_stock','date');

            $productRoomPriceLowLevel = DB::table('product_room_prices')->whereBetween('date', [$cart['arrival'], $cart['departure']])->where('status',1)->where('product_id',$room['product_id'])->lists('low_level','date');

            $date = $cart['arrival'];
            $end_date = $cart['departure'];

            // // udpate qty for product_room_prices
            $affected = DB::update("update product_room_prices set qty_stock = (qty_stock - 1), updated_at = '" . date('Y-m-d H:i:s') . "' where product_id = ? and date BETWEEN ? and ?", [$room['product_id'], $date, date('Y-m-d', strtotime($end_date. ' - 1 days'))]);
            // udpate done
            while (strtotime($date) <= strtotime($end_date)) {
                $roomBookedDateOrderIdsArr = DB::table('room_booked_date')->where('product_id',$room['product_id'])->whereRaw("('$date' between `date_checkin` and `date_checkout`)")->lists('order_id', 'id');

                if(!empty($roomBookedDateOrderIdsArr)){
                    $OrderProductQuantity = DB::table('order_to_product')->where('product_id', $room['product_id'])->whereIn('order_id', $roomBookedDateOrderIdsArr)->sum('quantity');
                    if($OrderProductQuantity>=(int) $productRoomPriceStockQty[$date]){
                        $OrderProductQuantityFinish['productArr'][$date][$room['product_id']] = $productArr;
                        $OrderProductQuantityFinish['productArr'][$date][$room['product_id']]['booked_stock_quantity'] = $OrderProductQuantity;
                        $OrderProductQuantityFinish['productArr'][$date][$room['product_id']]['stock_quantity'] = $productRoomPriceStockQty[$date];
                    }

                    if($OrderProductQuantity >= $productRoomPriceLowLevel[$date]){
                        $OrderProductQuantityLowArr['productArr'][$date][$room['product_id']] = $productArr;
                        $OrderProductQuantityLowArr['productArr'][$date][$room['product_id']]['booked_stock_quantity'] = $OrderProductQuantity;
                        $OrderProductQuantityLowArr['productArr'][$date][$room['product_id']]['low_level'] = $productRoomPriceLowLevel[$date];
                    }
        ...
        /// No Room Active
        if(!empty($OrderProductQuantityFinish)){
            $notification1 = new notification;
            $notificationsArr1 = $notification1->where('actions', 'like', '%room_active_for_booking%')->where('status',1)->get();
            foreach($notificationsArr1 as $notifications1){
                $sentEmailArr = array();
                $emailsArr = explode(',',$notifications1->emails);
                foreach($emailsArr as $email){
                    if(!in_array($email, $sentEmailArr)){
                        Mail::send('emails.room_active_for_booking', $OrderProductQuantityFinish, function ($message) use ($messageData, $email, $notifications1) {
                            $message->from($messageData['fromEmail'], $messageData['fromName']);
                            $message->to($email, $notifications1->name);
        ...
        $messageData = [
                'fromEmail' => 'registration@towerregency.com.my',
                'fromName' => 'Tower Regency Hotels & Apartments Online Booking',
            'subject' => 'Room running low::Order #' . $orderObj->order_id
        ];
        /// Room running low email notification
        if(!empty($OrderProductQuantityLowArr)){
            $notification2 = new notification;
            $notificationsArr2 = $notification2->where('actions', 'like', '%room_running_low%')->where('status',1)->get();
            foreach($notificationsArr2 as $notifications2){
                $sentEmailArr = array();
                $emailsArr = explode(',',$notifications2->emails);
                foreach($emailsArr as $email){
                    if(!in_array($email, $sentEmailArr)){
                        Mail::send('emails.room_running_low', $OrderProductQuantityLowArr, function ($message) use ($messageData, $email, $notifications2) {
                            $message->from($messageData['fromEmail'], $messageData['fromName']);
                            $message->to($email, $notifications2->name);
                            $message->subject($messageData['subject']);
                        });
                    }
                }
        ...
        $invoice['product_details'] = $ProductsModel->find($cart['product'][0]['product_id']);
        $invoice['discount'] = $orderObj->discount;
        ...
        $messageData = [
            ...
            'subject' => 'Tower Regency Hotel & Apartments::Order #' . $orderObj->id
        ];

        /* Send invoice emails to 3 other email addresses as well */
        //$messageData['toEmails'] = [$messageData['toEmail'],;//
        //$messageData['toEmails'] = [$messageData['toEmail'], "bruce@webqom.com"];
        //dd($invoice->cart);//
        ...
        $notification = new notification;
            ...
            foreach($notificationsArr as $notifications){
                ...
                foreach($emailsArr as $email){
                    if(!in_array($email, $sentEmailArr)){
                        Mail::send('invoice.admin-order-details-html', $invoice, function ($message) use ($messageData, $email, $notifications) {
                            $message->from($messageData['fromEmail'], $messageData['fromName']);
                            $message->to($email, $notifications->name);
                            $message->subject($messageData['subject']);
                        });
                    }
                }
