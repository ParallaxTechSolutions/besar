...
class Product extends Model{
	...
	*/
	...
	function getProductsByCategory($category_id, $sort = 'new', $filters = [],$get_total_records = null)
	{
		...
		if($sort == 'priceAsc'){
			...
			$query = $query->orderBy('order', 'ASC'); // has been added sort by order 4th Sept 2019 
			// $query = $query->orderBy('p.display_order', 'DESC'); // has been added sort by order 4th Sept 2019 
		}else{
			$query = $query->orderBy('p.id', 'DESC');
			// $query = $query->orderBy('p.display_order', 'ASC');
		}
		...
		else
		{
			...
			if($category_id)
			{
				...
				$results = $query->orderBy('order','asc')->groupBY('p.id')->skip($per_page * ($current_page-1))->take($per_page)->get();
				$results = $query->orderBy('order','asc')->groupBY('p.id')->skip($per_page * ($current_page-1))->take($per_page)->get();
			}
			else
			{
	...
	*/
	...
	public function getCartProducts($cartProducts, $promocode = array()){
		$productsInfo = array();
		$totalPrice = 0;
		$rooms = $cartProducts["rooms"];
		$adults = $cartProducts["adults"];
		$children = $cartProducts["children"];
		//echo "<pre>";print_r($cartProducts);exit;
		if($cartProducts){
			$arrival = $cartProducts["arrival"];	
			$departure = $cartProducts["departure"];
			
			$i = 0;
			foreach($cartProducts["product"] as $key => $cartProduct){

				//get infomation of product
				if (isset($cartProduct['pwp_product_id'])) {
					$product = $this->getProduct($cartProduct['pwp_product_id']);
				} else {
					$product = $this->getProduct($cartProduct['product_id'],$cartProducts['arrival'],$cartProducts['departure']);
				}
				...
				if($product){
					...
					//End color
					$productsInfo[$i]->cart['date_checkin'] = $arrival;	
					$productsInfo[$i]->cart['date_checkout'] = $departure;
					//Get special events
					$eventName = false;
					$eventToken = false;
			...
			foreach($productsInfo as $key => $product){
				$quantityDiscount = $this->getProductToQuantityDiscount($product->id, $product->cart['qty']);
