...
class Users extends Model
{
    ...
    function insertregistereddata($formData)
    {
        ...
        if (isset($formData['billing_last_name']) && $formData['billing_last_name'] != '') {
            $billing_last_name = $formData['billing_last_name'];
        } else {
            $billing_last_name = '';
        }
        if (isset($formData['billing_telephone'])) {
            $billing_telephone = $formData['billing_telephone'];
        } else {
            $billing_telephone = '';
        }
        ...
        if (isset($formData['newsletter_subscription'])) {
        ...
        if (isset($formData['billing_address'])) {
        ...
        if (isset($formData['billing_post_code'])) {
            $billing_post_code = $formData['billing_post_code'];
        } else {
            $billing_post_code = '';
        }
        ...
        $this->sendNotificationOnClientRegistration($data);
        return true;
    }
    ...
    function sendregistrationmail($formData)
    {
        ...
        $subject = "Congratulations! You have successfully registered with Tower Regency Hotel & Apartments.";
        #registration
        $url = URL::to('/');
...
//        $data['formData']['email'] = 'parmarvikrantr@gmail.com';//$mail;
    ...
    function sendsubscriptionmail($formData)
    {
        ...
        $messageData = [
            ...
            'fromName' => 'Tower Regency Hotel & Apartments Online Registration Manager',
    ...
    function updateAccount($formData)
    {
        ...
        if (isset($formData['birth_date'])) {
            ...
            $birth_date = date('Y-m-d H:i:s', ($old_date));
        } else {
            $birth_date = '';
        }
    ...
    {
        if (isset($formData['shipping_first_name'])) {
            $shipping_first_name = $formData['shipping_first_name'];
        } else {
            $shipping_first_name = '';
        }
        if (isset($formData['shipping_last_name']) && $formData['shipping_last_name'] != '') {
            $shipping_last_name = $formData['shipping_last_name'];
        } else {
            $shipping_last_name = '';
        }
        if (isset($formData['shipping_telephone'])) {
            $shipping_telephone = $formData['shipping_telephone'];
        } else {
            $shipping_telephone = '';
        }

        if (isset($formData['shipping_email'])) {
            $shipping_email = $formData['shipping_email'];
        } else {
            $shipping_email = '';
        }
        if (isset($formData['shipping_address'])) {
            $shipping_address = $formData['shipping_address'];
        } else {
            $shipping_address = '';
        }
        if (isset($formData['shipping_city'])) {
            $shipping_city = $formData['shipping_city'];
        } else {
            $shipping_city = '';
        }
        if (isset($formData['shipping_post_code'])) {
            $shipping_post_code = $formData['shipping_post_code'];
        } else {
            $shipping_post_code = '';
        }
        if (isset($formData['shipping_state'])) {
            $shipping_state = $formData['shipping_state'];
        } else {
            $shipping_state = '';
        }
        if (isset($formData['shipping_country'])) {
    ...
    //////Ger all order history
    function getUserPartnerAllOrder($customerId, $sort, $page, $item)
    {
        ...
        $result = DB::select("select * from orders where partner_id = '" . $customerId . "' and createdate > '" . $createdate . "' order by id desc limit " . $startLimit . ", " . $item);
        return $result;
    }
    ...
    //////Get total sort order history
    function getUserPartnerAllSortOrder($customerId, $sort)
    {
        ...
        $result = DB::select("select * from orders where partner_id = '" . $customerId . "' and createdate > '" . $createdate . "' order by id desc  ");
        return $result;
    }
    ...
    public function getOrderToProduct($order_id)
    {
        return DB::table('order_to_product as otp')
            ->select('otp.*', 'p.*', 'p.type as product_name', 'p.room_code as product_code', 'p.thumbnail_image_1', 'p.thumbnail_image_2', 'c.title as color_name', 'p.property_id', 'pr.name')
            ->leftjoin('colors as c', 'c.id', '=', 'otp.color_id')
            ->leftjoin('products as p', 'p.id', '=', 'otp.product_id')
            ->leftjoin('property as pr', 'pr.property_id', '=', 'p.property_id')
            ->where('otp.order_id', $order_id)
            ->groupBy('otp.product_id') // later added
            ->get();
    }

    ////Send email for new client registeration
    public function sendNotificationOnClientRegistration($formData)
    {
        $url = URL::to('/');
        $url .= "/public/front/images/index/logo.png";
        $messageData = [
        ...
        $formData['url'] = $url;
        $billingCountryNameArr = DB::table('countries')->where('country_id', $formData['billing_country'])->select('name')->first();
        $formData['billing_country_name'] = $billingCountryNameArr->name;
        $billingStateNameArr = DB::table('states')->where('zone_id', '=', $formData['billing_state'])->select('name')->first();
        $formData['billing_state_name'] = $billingStateNameArr->name;
        //dd($formData);

        $notification = new notification;
        $notificationsArr = $notification->where('actions', 'like', '%client_registered%')->where('status', 1)->get();
        ...
        foreach ($notificationsArr as $notifications) {
            ...
            foreach ($emailsArr as $email) {
                if (!in_array($email, $sentEmailArr)) {
                    Mail::send('emails.clientRegisteration', ['formData' => $formData], function ($message) use ($messageData, $email, $notifications) {
                        $message->from($messageData['fromEmail'], $messageData['fromName']);
...
//////Get user detail by ID from DB table
    function getPartnerUserById($id)
    {
...
//////Ger order for use for dashboard
    function getPartnerUserOrder($partnerId)
    {
        $result = DB::table('orders')->where('partner_id', $partnerId)->orderBy('id', 'desc')->take(5)->get();
        return $result;
    }
