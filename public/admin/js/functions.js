...
function delete_selected(item_type)
{
	...
	if(item_type == 'property')
		continue_delete_process_property();

	if(item_type == 'drop_off_list')
		continue_delete_process_drop_list();
...
function continue_delete_process_property(){
	item_id = $('#delete_item_ids').val();
	...
	$.ajax({
		   type : 'POST',
		   url : base_url+'web88cms/property/deleteproperties',
		   data : 'item_id='+item_id+'&_token='+$('#csrf_token').val(),
		   success : function(response){
		   		if(query_string == '')
					window.location.href = base_url+"web88cms/property";
				else
					window.location.href = base_url+"web88cms/property?"+query_string;
		   }
	});	// end ajax
}
function continue_delete_process_drop_list(){
	item_id = $('#delete_item_ids').val();
	...
	$.ajax({
		   type : 'POST',
		   url : base_url+'web88cms/delete-drop-off-list',
		   data : 'item_id='+item_id+'&_token='+$('#csrf_token').val(),
		   success : function(response){
		   		if(query_string == '')
					window.location.href = base_url+"web88cms/drop-off-list";
				else
					window.location.href = base_url+"web88cms/drop-off-list?"+query_string;
		   }
	});	// end ajax
}
